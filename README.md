...
**混淆事件未完全剔除：**受限于数据收集管线，本研究未能系统性剔除与季度财报发布日（Earnings Announcements）重叠的裁员事件。科技公司常在财报电话会议中同步宣布裁员，此时短窗口的 CAR 可能更多反映了市场对 EPS 超预期/不及预期的反应，而非单纯对裁员决定的定价。未控制此类混淆事件可能会为短窗口事件研究引入较大的噪音。


**样本筛选规则的潜在选择性偏差：**在构建成熟公司样本时，本研究出于数据可得性，剔除了裁员期间面临退市风险或股价长期跌破 1 美元的困境公司。尽管这提高了样本的公司治理基准，但这种基于事后（ex-post）表现的剔除规则可能引入一定程度的幸存者偏差，使得全样本的平均异常收益被轻微向均值上方拉动。未来的改进应当采用基于事前（ex-ante）市值的条件进行过滤。
